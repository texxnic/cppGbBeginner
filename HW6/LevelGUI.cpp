...
void LevelGUI::Draw() const
{
    ...
    GotoXY(62, 1);
    cout << "Score: " << score;
    if (timeTillMessage_ > 0 && !messages_.empty())
        PrintMessage();
}
...
void LevelGUI::PrintMessage() const
{
    GotoXY(8, GetMaxY() - 3);
    cout << messages_.front();
}
...
void __fastcall LevelGUI::SetParam(uint64_t passedTimeNew, uint64_t fpsNew, uint16_t bombsNumberNew, int16_t scoreNew)
{
    ...
    score = scoreNew;
    timeTillMessage_ -= GetTickCount64();
    if (timeTillMessage_ <= 0)
    {
        messages_.pop();
        timeTillMessage_ = 3000;
    }
}